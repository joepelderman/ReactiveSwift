...
internal class SignalProducerCore<Value, Error: Swift.Error> {
	...
	func makeInstance() -> Instance {
		...
		let (interrupter, start) = bootstrap(observer)
		disposable.inner = interrupter

		return Instance(signal: signal,
						observerDidSetup: start,
						interruptHandle: disposable)
	}

	/// Bootstrap a new producer instance with the given observer.
	///
	/// - important: The callee is responsible for starting the producer
	///              instance by invoking the returned `start` handle.
	///
	/// - parameters:
	///   - observer: The observer to receive events from the bootstrapped
	///               producer instance.
	///
	/// - returns: The interrupter of the producer instance, and a handle to
	///            start the producer instance.
	@discardableResult
	func bootstrap(_ observer: Signal<Value, Error>.Observer) -> (interrupter: Disposable, start: () -> Void) {
		fatalError()
	}
...
private final class SignalCore<Value, Error: Swift.Error>: SignalProducerCore<Value, Error> {
	...
	@discardableResult
	override func bootstrap(_ observer: Signal<Value, Error>.Observer) -> (interrupter: Disposable, start: () -> Void) {
...
private final class TransformerCore<Value, Error: Swift.Error, SourceValue, SourceError: Swift.Error>: SignalProducerCore<Value, Error> {
	...
	override func bootstrap(_ observer: Signal<Value, Error>.Observer) -> (interrupter: Disposable, start: () -> Void) {
		...
		let (upstreamInterrupter, upstreamStart) = source.bootstrap(Signal.Observer(input))
...
extension SignalProducer {
	...
	public func start(_ observer: Signal<Value, Error>.Observer = .init()) -> Disposable {
		let (interrupter, start) = core.bootstrap(observer)
		start()
		return interrupter
	}
